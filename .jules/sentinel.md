...
## 2026-12-15 - Fail-Safe Security Header Consistency
**Vulnerability:** Security headers like `Permissions-Policy` and `X-Robots-Tag` were omitted in certain failure paths (e.g., rate-limit blocks, global 500 errors) or in synthetic responses generated by the Service Worker, creating a security posture disparity.
**Learning:** Hardening the standard request-response path is insufficient for defense-in-depth. Volumetric attacks (429) and server exceptions (500) bypass standard route middleware, and Service Workers act as a secondary response layer that must mirror the server's security configuration.
**Prevention:** Implement a "fail-safe" header pattern by (1) registering security middleware at the very top of the Express stack, (2) explicitly duplicating security headers in global error and 404 handlers, and (3) maintaining strict parity between server-side and Service Worker response headers.
