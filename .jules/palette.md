...
## 2026-06-20 - [Safe Markdown Links & Actionable Feedback]
**Learning:** Supporting Markdown links `[text](url)` alongside bare URL detection requires a placeholder strategy to prevent double-wrapping and corrupted HTML attributes. Furthermore, providing clear visual affordance for external links (e.g., a `↗` icon) and enriching transient feedback with icons (e.g., `📋 Copied`) improves both usability and perceived quality. Finally, ensuring dynamic interactive elements like "Undo" buttons carry explicit `title` and `aria-label` attributes is vital for bridging the gap between mouse-driven discoverability and screen reader clarity.
**Action:** Use placeholder-based restoration for multi-pass text-to-HTML conversions; always provide visual cues for external navigation; audit dynamic buttons for shortcut discoverability and semantic labels.

## 2026-06-27 - [Contextual Accessibility & Temporal Truthfulness]
**Learning:** In fast-moving task queues, generic action labels (e.g., "Approve") lose meaning when disconnected from the content. Injecting the item title into dynamic `aria-label` and `title` attributes (e.g., "Approve 'Fix CSS bug'") significantly reduces cognitive load and improves screen reader clarity. Furthermore, relative timestamps (e.g., "just now") in backgrounded tabs become misleading; refreshing them immediately on `visibilitychange` focus ensures the UI remains "truthful" without requiring a full manual refresh.
**Action:** Always provide contextual identifiers in primary action buttons and notifications; pair relative time intervals with an immediate focus-triggered refresh.
